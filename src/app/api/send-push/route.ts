...
// We use the anon key but call SECURITY DEFINER RPCs to bypass RLS
...
export async function POST(request: NextRequest) {
  try {
    ...
    // but we'll use SECURITY DEFINER RPCs for data access
    ...
    // 3. Resolve special keywords like 'admins' or 'supervisors' using SECURITY DEFINER RPC
    ...
    if (needsAdmins || needsSupervisors) {
      ...
      const { data: roleUsers, error: roleError } = await supabaseWithAuth
        .rpc('get_user_ids_by_roles', { p_roles: rolesToFetch });

      if (roleError) {
        console.error('Error resolving role-based user IDs:', roleError);
    ...
    // 4. Fetch active subscriptions using SECURITY DEFINER RPC
    const { data: subscriptions, error: dbError } = await supabaseWithAuth
    ...
    const sendPromises = subscriptions.map(async (sub: any) => {
      ...
      try {
        ...
        console.warn(`Failed to send push notification to subscription ${sub.sub_id}:`, err.message || err);
        
        // Purge subscription if it's expired or gone (404 / 410)
        if (err.statusCode === 410 || err.statusCode === 404) {
          console.log(`Purging inactive subscription ${sub.sub_id}`);
          await supabaseWithAuth.rpc('delete_push_subscription', { p_sub_id: sub.sub_id });
        }
